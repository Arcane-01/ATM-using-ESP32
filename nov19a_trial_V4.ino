...
const char* ssid = "ArcAne3042";
const char* password = "2574)8gU"; //1565+qD7

// Initialize Telegram BOT
#define BOTtoken "5966245620:AAFDir2fvXxtVgKTjbBuHTAZfuQn7pfRg6w"  // your Bot Token (Get from Botfather)
// The user's chat id
...
String GOOGLE_SCRIPT_ID = "AKfycbxx9X5f0GwE9VFjX_QFs_JFC7qlBHYxbQmTsgHYICgesHXbSSmjoX_dFNBN7dD3Ucx9";    // change Gscript ID
...
void handleNewMessages(int numNewMessages) {
  ...
  // Start Function: Prompts the user to various available functions
    ...
    //Code for Withdrawal
      ...
      Serial.println(result);
      if (result == "1"){
        String keyboardJson = "[[\"/Debit\", \"/Credit\"],[\"/Balance_Inquiry\",\"/PIN_Change\"],[\"/Fund_Transfer\",\"/Exit\"]]";
    ...
    // Code for Credit
      ...
      if(Logged_in == true){
        bot.sendMessage(chat_id, "Enter amount to be credited", "");
      }
      else if (Logged_in == false) {
        bot.sendMessage(chat_id, "The user is not Logged in", "");
      }
      ...
      Serial.println(result);
      String keyboardJson = "[[\"/Debit\", \"/Credit\"],[\"/Balance_Inquiry\",\"/PIN_Change\"],[\"/Fund_Transfer\",\"/Exit\"]]";
      bot.sendMessageWithReplyKeyboard(chat_id, "Rs. "+amount+" credited successfully.", "", keyboardJson, true);
    }
      ...
      String msg3 = "You can choose one of the following options,\n";
      msg3 += "/Enter_PIN - to enter the password for logging in\n\n";
      msg3 += "/Exit - to exit the entire process \n\n";
      bot.sendMessageWithReplyKeyboard(chat_id, msg3, "", keyboardJson, true);
      ...
      if(v==0){
        ...
        msg += "/Fund_Transfer - to transfer money from your bank account to some other user";
        Logged_in = true;
        String keyboardJson = "[[\"/Debit\", \"/Credit\"],[\"/Balance_Inquiry\",\"/PIN_Change\"],[\"/Fund_Transfer\",\"/Exit\"]]";
        bot.sendMessageWithReplyKeyboard(chat_id, msg, "", keyboardJson, true);
      }
      if(v==1){
        String keyboardJson = "[[\"/Enter_PIN\"],[\"/Exit\"]]";
        String msg2 = "Incorrect Password. Select one of the following options:\n";
        msg2+= "\Enter_PIN - to re-enter the password";
        msg2+="\Exit - to exit the process";
        bot.sendMessageWithReplyKeyboard(chat_id, msg2, "", keyboardJson, true);
        PassPIN = "";
        text = "";
        prev_text = "";
        FlagU = false;
      ...
      if(Logged_in == true){
        String keyboardJson = "[[\"/Exit\"]]";
        bot.sendMessageWithReplyKeyboard(chat_id, "Enter the reciever's name to whom the money is to be transferred", "", keyboardJson, true);
      }
      else if (Logged_in == false) {
        String keyboardJson = "[[\"/Login\", \"/New_User\"],[\"/Exit\"]]";
        bot.sendMessageWithReplyKeyboard(chat_id, "You are not logged in", "", keyboardJson, true);
      }
    }
      ...
      String keyboardJson = "[[\"/Exit\"]]";
      bot.sendMessageWithReplyKeyboard(chat_id, "Enter the amount to be transferred", "", keyboardJson, true);
      FlagFT = true;
    }
    else if (FlagFT == true){
      FlagFT = false;
      ...
      String url_write = "https://script.google.com/macros/s/" + GOOGLE_SCRIPT_ID + "/exec?sender="+ String(user) +"&reciever="+String(reciever) +"&fund="+String(funds);
      ...
      if(result=="0"){
        String keyboardJson = "[[\"/Debit\", \"/Credit\"],[\"/Balance_Inquiry\",\"/PIN_Change\"],[\"/Fund_Transfer\",\"/Exit\"]]";
        bot.sendMessageWithReplyKeyboard(chat_id, "Insufficient funds", "", keyboardJson, true);
      }
      if(result=="1"){
        String keyboardJson = "[[\"/Debit\", \"/Credit\"],[\"/Balance_Inquiry\",\"/PIN_Change\"],[\"/Fund_Transfer\",\"/Exit\"]]";
        bot.sendMessageWithReplyKeyboard(chat_id, "Successfully transferred Rs. "+funds+ " from " + user + " to "+reciever , "", keyboardJson, true);
      }
      if(result=="2"){
        String keyboardJson = "[[\"/Debit\", \"/Credit\"],[\"/Balance_Inquiry\",\"/PIN_Change\"],[\"/Fund_Transfer\",\"/Exit\"]]";
        bot.sendMessageWithReplyKeyboard(chat_id, "No such username found for reciever", "", keyboardJson, true);
      }
      reciever = "";
      funds = "";
    }
